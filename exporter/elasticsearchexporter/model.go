...
// encoder is an interface for encoding signals to Elasticsearch documents.
...
func newEncoder(mode MappingMode) (encoder, error) {
	switch mode {
	case MappingNone:
...
func newDataPointHasher(mode MappingMode) (dataPointHasher, error) {
	switch mode {
	case MappingOTel:
		return otelDataPointHasher{}, nil
	case MappingECS:
		// TODO raw/none?
		return ecsDataPointHasher{}, nil
	}
	return nil, fmt.Errorf("mapping mode %q (%d) does not support metrics", mode, int(mode))
}
...
func (e otelModeEncoder) encodeLog(
	...
	return serializeLog(ec.resource, ec.resourceSchemaURL, ec.scope, ec.scopeSchemaURL, record, idx, buf)
}
...
func (otelModeEncoder) encodeSpan(
	...
	return serializeSpan(ec.resource, ec.resourceSchemaURL, ec.scope, ec.scopeSchemaURL, span, idx, buf)
}
...
func (otelModeEncoder) encodeSpanEvent(
	...
	serializeSpanEvent(ec.resource, ec.resourceSchemaURL, ec.scope, ec.scopeSchemaURL, span, spanEvent, idx, buf)
	return nil
}
...
func (otelModeEncoder) encodeMetrics(
	...
	return serializeMetrics(ec.resource, ec.resourceSchemaURL, ec.scope, ec.scopeSchemaURL, dataPoints, validationErrors, idx, buf)
}
