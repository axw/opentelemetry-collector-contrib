...
func TestNewSaramaClientConfig(t *testing.T) {
	for name, tt := range map[string]struct {
		...
		"metadata": {
			...
			check: func(t *testing.T, cfg *sarama.Config) {
				assert.Equal(t, false, cfg.Metadata.Full)
		...
		"auth": {
			...
			expectedErr: `error loading tls config: failed to load TLS config: failed to load CA CertPool File: failed to load cert /nonexistent: open /nonexistent: no such file or directory`,
		},
	} {
